...
void writeWave(ofstream & ofs, int time, double xi[nx][nt]) {
      ...
      if(kt>0) {
         ofs<<"  "<<endl;    // two empty lines for animation.
         ofs<<"  "<<endl;
      }
      ...
      for(int i=0; i<nx; i++) {
         double tval=kt;
         double xval=i;    //  100 cm long stirng
         double yval=xi[i][kt];       //  string displacement
...
void wave1Dpropagate(double xi[nx][nt],double ratio,int tmax){
   ...
   ofstream outfileA;
   outfileA.open("wave1Dfull.txt");

   ofstream outfileB;
   outfileB.open("wave1Dfirst.txt");
